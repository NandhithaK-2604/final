...
if MODEL_ERRORS:
    with st.sidebar.expander("Model load status"):
        for k, err in MODEL_ERRORS.items():
            st.write(f"- {k}: {err}")
# -------------------------
# Helper functions used by Test & Predict
# -------------------------
def audio_to_chunks_bytes(file_bytes, chunk_duration=CHUNK_DURATION, sr=SR):
    """Return list of chunks (numpy arrays) and sr from raw file bytes."""
    try:
        # librosa accepts a file-like object
        y, sr_ret = librosa.load(io.BytesIO(file_bytes), sr=sr)
        # optional denoising (keeps behaviour consistent with existing code)
        try:
            y = nr.reduce_noise(y=y, sr=sr_ret)
        except Exception:
            pass
        chunk_len = int(chunk_duration * sr_ret)
        chunks = [y[i : i + chunk_len] for i in range(0, len(y), chunk_len) if len(y[i : i + chunk_len]) == chunk_len]
        return chunks, sr_ret
    except Exception as e:
        st.error(f"Error reading audio for chunking: {e}")
        return [], sr

def chunk_to_spectrogram_buf(chunk, sr, n_mels=128, hop_length=512):
    """Return an in-memory PNG buffer with the mel-spectrogram of chunk."""
    try:
        S = librosa.feature.melspectrogram(y=chunk, sr=sr, n_mels=n_mels, hop_length=hop_length)
        S_db = librosa.power_to_db(S, ref=np.max)
...
def preprocess_spectrogram_buf_to_model_input(img_buf, target_size=IMG_SIZE):
    """Load PNG buffer into Keras-style input (1,H,W,3), normalized to [0,1]."""
    try:
        # Use Keras image loader which was already imported as image
        img = image.load_img(io.BytesIO(img_buf.getvalue()), target_size=target_size)
        arr = image.img_to_array(img) / 255.0
        arr = np.expand_dims(arr, axis=0)
        return arr
    except Exception as e:
        st.error(f"Spectrogram preprocessing error: {e}")
        return None


# -------------------------
# Test & Predict page (replacement)
# -------------------------
if page == "Test & Predict":
    st.header("Upload a WAV audio file for prediction")
    ...
    uploaded = st.file_uploader("Choose a .wav file", type=["wav"])
    if not uploaded:
        st.info("Upload a WAV file (≥ 3 seconds) to extract features, view spectrogram, and run model predictions.")
    else:
        audio_bytes = uploaded.read()
        st.audio(audio_bytes)  # playback

        # 1) Extract features (compact)
        y, sr = safe_load_audio(io.BytesIO(audio_bytes))
        if y is not None:
            y_nr = reduce_noise(y, sr)
            extracted = extract_basic_features_from_audio(y_nr, sr)
            ...
            kdf = pd.DataFrame([{c: extracted.get(c, np.nan) for c in key_cols}]).T
            kdf.columns = ["Value"]
            st.subheader("Key extracted features (compact)")
            st.dataframe(kdf.round(4), width=520)

            # full features in an expander (compact)
            with st.expander("Show full feature set (compact)"):
                items = list({c: extracted.get(c, np.nan) for c in FEATURE_COLUMNS}.items())
                half = math.ceil(len(items) / 2)
                c1_items, c2_items = items[:half], items[half:]
            ...
            st.error("Unable to read audio to extract features. Try another file.")
            # skip prediction attempt if audio couldn't be loaded
            st.stop()

        # 2) Chunk the audio and produce spectrogram for first chunk
        chunks, sr_chunks = audio_to_chunks_bytes(audio_bytes, chunk_duration=CHUNK_DURATION, sr=SR)
        st.write(f"Audio split into {len(chunks)} chunk(s) of {CHUNK_DURATION}s")
        if len(chunks) == 0:
            st.warning(f"No full {CHUNK_DURATION}s chunk found. Use a longer audio file or reduce CHUNK_DURATION.")
        else:
            first_chunk = chunks[0]
            spec_buf = chunk_to_spectrogram_buf(first_chunk, sr_chunks, n_mels=128, hop_length=512)
            if spec_buf is None:
                st.error("Failed to generate spectrogram for the chunk.")
            else:
                

                # 3) Preprocess and run predictions using loaded MODELS
                x_input = preprocess_spectrogram_buf_to_model_input(spec_buf, target_size=IMG_SIZE)
                if x_input is None:
                    st.error("Failed to preprocess spectrogram for model input.")
                else:
                    # Ensure models exist
                    if not MODELS:
                        st.warning("No models were loaded (MODELS is empty). Check the 'Model load status' in the sidebar.")
                    else:
                        st.subheader("Model predictions")
                        results = {}
                        for name, model in MODELS.items():
                            try:
                                ...
                                if isinstance(out, np.ndarray):
                                    if out.ndim == 2 and out.shape[-1] > 1:
                                        # multiclass logits/probabilities - take class-1 probability if present
                                        try:
                                            score = float(out[0][1])
                                        except Exception:
                                            # fallback: take second column if exists otherwise first
                                            score = float(out[0][-1])
                                        ...
                                        # last fallback: try scalar conversion
                                        score = float(np.asarray(out).flatten()[0])
                                else:
                                    score = float(out)
                                label = "Parkinson's Disease" if score >= 0.5 else "Healthy Control"
                                results[name] = {"Score": round(score, 4), "Prediction": label}
                        ...
                        # Show results as a neat DataFrame
                        preds_df = pd.DataFrame(results).T
                        preds_df.index.name = "Model"
                        # if any scores are None, replace with NaN for nicer display
                        preds_df["Score"] = preds_df["Score"].astype(object).apply(lambda v: np.nan if v is None else v)
                        st.table(preds_df)
